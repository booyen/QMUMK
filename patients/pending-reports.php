...
<div class="panel-heading theme-patients"><span class="inlineicon pending-mini">Senarai pesakit menuggu giliran</span></div>
...
<ol class="breadcrumb link-patients">
  ...
  <li><a href="../patients/">Pesakit</a></li>
   <i class="material-icons bread-icon">navigate_next</i> 
  <li class="active">Senarai menunggu</li>
</ol>

	<?php if(display_permission("prescribe_patient")==true){ ?>
	<p class="pull-right in-page-nav">
	  <a href="pending-reports.php" role="button" class="btn btn-link <?php if($show_pending=='global'){ echo "theme-patients"; }?> btn-sm">Keselurahan</a>
    ...
    <thead>
    <tr><td>No. Giliran ID#</td><td width="30%">Jenis kesakitan</td><td></td><td>Kategori pesakit</td><td>Status</td><td>Tarikh</td></tr>
	</thead>
	...
	<tr>
		<td><?php echo $reports['id']?></td>
			<td class="symptoms">
		<a href="../patients/reports.php?id=<?php echo $reports['id']?>"><?php echo substr($reports['symptoms'],0,80);?>...</a>
		</td>
		<td class="branch-name">
			<?php if($reports['branch']!=staff_info('branch')) echo "[".branch_info('name',$reports['branch'])."]";?></td>
		<td class="status"><?php if($reports['signed_by']!=""){?>

		<span class="s"><?php echo staff_info("full_name",$reports['signed_by']);?></span>

		<?php }?><?php if($reports['signed_by']==""&&$reports['engaged_by']!=""){?>

		<span class="e">(Assigned)</span>

		<?php }?><?php if($reports['signed_by']==""&&$reports['engaged_by']==""){?>

		<span class="p">(Menuggu)</span>
	...
	<?php if($tcount==0){?>
	<h4 class="text-center">Tiada pesakit yang sedang menunggu!</h4>
	<?php }?>
