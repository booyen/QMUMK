...
<ol class="breadcrumb link-patients">
  ...
  <li class="active">Pesakit</li>
</ol>

	<?php if(isset($_GET['deleted'])){$deleted=$_GET['deleted'];?>
    <div class="alert alert-success" role="alert">Rekod pesakit telah berjaya dipadam!</div>
	<?php }?>

	<h4 class="patients-dir-total" align="right">Jumlah pesakit yang berdaftar: <?php echo $total_items;?>  <?php if($start!=0){?> <a href="?start=<?php echo $start-$limit; ?>">previous page</a>  <?php }?><?php if($total_items>=$start+$limit){?><a href="?start=<?php echo $start+$limit; ?>">next page</a><?php }?></h4>
