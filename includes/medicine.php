...
function introduce_medicine($category,$code,$name,$price,$frequency,$condition,$usage,$added_by){
	global $con;
	...
	$getting_match=mysqli_query($con, "select * from p_medicine_dir where code='$code'");
	 if(mysqli_num_rows($getting_match)>0){
		...
		mysqli_query($con, "insert into p_medicine_dir set category='$category',code='$code',name='$name',price='$price',frequency ='$frequency',med_condition='$condition',med_usage='$usage',added_by='$added_by',last_update='$current_time' ") 
		or die(mysqli_error());
		return true;
	 }
}
